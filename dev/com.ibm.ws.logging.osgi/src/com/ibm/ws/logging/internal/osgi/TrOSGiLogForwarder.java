...
public class TrOSGiLogForwarder implements SynchronousLogListener, SynchronousBundleListener {
        ...
        synchronized (traceComponents) {
            ...
            if (tc == null) {
                ...
                if (bsn.endsWith("org.eclipse.equinox.metatype")) {
                    ...
                    if (group == null) {
                        tc = new OSGiTraceComponent(logName, this.getClass(), new String[] { bsn, OsgiLogConstants.LOG_SERVICE_GROUP }, ffdcMe);
                    } else {
                        tc = new OSGiTraceComponent(logName, this.getClass(), new String[] { bsn, group, OsgiLogConstants.LOG_SERVICE_GROUP }, ffdcMe);
                    }
                }
	...
	public void logged(LogEntry le) {
        ...
        try {
        	if (isAnyTraceEnabled && tc.isEventEnabled() && logEntry.getContext() instanceof EventObject) {
        		String loggerName = logEntry.getLoggerName();
                if(LOGGER_FRAMEWORK_EVENT.equals(loggerName) || LOGGER_BUNDLE_EVENT.equals(loggerName) || LOGGER_SERVICE_EVENT.equals(loggerName)) {
