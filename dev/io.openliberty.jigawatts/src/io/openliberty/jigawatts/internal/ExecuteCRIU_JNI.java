...
public class ExecuteCRIU_JNI implements ExecuteCRIU {
    ...
    public SnapshotResult dump(File directory) {
        try {
            ...
            snapshotResult = new SnapshotResult(SnapshotResultType.SUCCESS, "Success", null, SnapshotResultType.SUCCESS.getCode());
        } catch (IOException e) {
            snapshotResult = new SnapshotResult(SnapshotResultType.SNAPSHOT_FAILED, "The criu dump command failed with error: Snapshot Failed", e, SnapshotResultType.SNAPSHOT_FAILED.getCode());
        }
    }
