...
public class TrConfigurator {
    ...
    static StaticValue<TrService> delegate = null;
    ...
    public static synchronized void init(LogProviderConfig config) {
        ...
        if (loggingConfig.get().compareAndSet(null, config)) {
            ...
            if (delegate == null) {
                delegate = StaticValue.createStaticValue(result);
            }
            delegate.computeIfAbsent(result).init(config);
    ...
    static TrService getDelegate() {
        StaticValue<TrService> trDelegate = delegate;
        if (trDelegate != null)
            return trDelegate.get();
